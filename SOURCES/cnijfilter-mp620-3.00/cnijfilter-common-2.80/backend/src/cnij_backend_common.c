...
//static int  make_lgmon_command( char *, char* ) ;
static int  make_lgmon_command( char *, char*, int ) ;
static int  lgmon_exec( int, FILE *, int , char *, int ) ;
static void data_write( int, FILE *, int , int ) ;

static int check_options( FILE**, short*, int*, char*, char* );
...
*/
	...
	if( option != NULL )
		check_options( &fp, &is_setregi, &utilbyte, tmp_file, option );

	sts = make_lgmon_command( lgmon_command, option, utilbyte ) ;
...
static
int	make_lgmon_command( char *command, char *option, int utilbyte )
{
