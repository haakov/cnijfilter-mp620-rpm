...
{
	char	small_tmpname[64],useagebjfilter[64],useagebjfiltergui[64];
	short	i;

	for( i=0; i<sizeof(small_tmpname); i++)
		small_tmpname[i] = tolower(modelname[i]);

	snprintf(useagebjfiltergui, sizeof(useagebjfiltergui),"%s%s%s",usage_bjfiltergui,small_tmpname,usage_guimode);
	...
	char 				*modelname )
{
	...
	if (setopt[OPTINDEX(OPTGUI)] & OPTBIT(OPTGUI)) {
		...
		if (setopt[OPTINDEX(OPTDISPLAY)] & OPTBIT(OPTDISPLAY)) {
			strncpy(dispname, opt->display, 256);
			dispname[255] = '\0';
		}
		else {
			strncpy(dispname, "localhost:0.0", 256);
			dispname[255] = '\0';
		}
		return 0;
	}
