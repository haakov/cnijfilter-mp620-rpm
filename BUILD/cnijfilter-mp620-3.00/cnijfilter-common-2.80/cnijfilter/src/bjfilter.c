...
#if 0
static void print_ierr2(long err)
{
	fprintf ( stderr, "Internal error 2 : %ld %ld %ld \n", 
		BJERR_GET_TYPE(err),
		BJERR_GET_FILE(err),
		BJERR_GET_LINE(err) ) ;
}
...
int main( int argc, char *argv[] )
{
	...
	short				id = 0;
	short				testprint_ret;
	short				result;		/* return value of ForkLgmon() */
	DATA_INFO			data_info;
		...
		if ( lpbjinfo->pid_printui == 0 ){
			...
			while( 1 ){
				...
				if ( ret == RET_PRINT ){
					...
					if ( ( result =  ForkLgmon( lpbjinfo, dispname, modelname ) ) < 0 ){
						fprintf( stderr, "ForkLgmon Error\n" );
						goto onErr;
					}
					
					/* send header to LM if LM exists */
					if( result ){	/* send header to LM */
						data_info.type = PRINT_DATA;
						data_info.size = 0L;
						outCmd( (CPKByte CPKPTR)(&data_info), (CPKUInt32)sizeof(DATA_INFO), lpbjinfo->prn );
					}
					...
					lpbjinfo->bjfoption.stapleside = ipc.parm.bjflt_uisetup.bjflt_stapleside;
					...
					/* send header to LM if LM exists */
					if( result ){	/* send header to LM */
						data_info.type = UTIL_DATA;
						data_info.size = ipc.cmds.cmdslen;						
						outCmd( (CPKByte CPKPTR)(&data_info), (CPKUInt32)sizeof(DATA_INFO), lpbjinfo->prn );
					}
				...
				else if ( ret == RET_FDATA ){		/* testprint */
					/* check_lgmon_status(lpbjinfo, dispname, modelname); */
					if ( ( result =  ForkLgmon( lpbjinfo, dispname, modelname ) ) < 0 ){
						fprintf( stderr, "ForkLgmon Error\n" );
						goto onErr;
					...
					/* send header to LM if LM exists */
					if( result ){	/* send header to LM */
						data_info.size = get_file_bytes( ipc.fncmds.filename );
						if( data_info.size < 0 ){
							fprintf( stderr, "Maintenance file Error\n" );
							goto onErr;
						}
						
						data_info.size +=  ipc.fncmds.cmdslen;		
						data_info.type = UTIL_DATA;
					...
					/* send BJL to LM */
					/* open testpattern file and write printdata to LM */
					if( ( testprint_ret = exec_testprint( ipc.fncmds.cmds , ipc.fncmds.cmdslen , ipc.fncmds.filename , lpbjinfo->prn)) < 0  ){
						if( testprint_ret == -2 ){
							fprintf( stderr, "Maintenance file Error\n" );
						}
						goto onErr;
...
static short ForkLgmon( LPBJFILTERINFO lpbjfinfo, char *dispname, char *modelname )
{
	char *arg_noui[] = { LGMONPATH, "--model", "--header" , modelname, NULL };
	char *arg_ui[] = { LGMONPATH, "--gui", "--display", dispname, "--model", "--header", modelname, NULL };
	...
	if ( ( cnt = strlen(modelname) ) >= sizeof(small_modelname) )
		cnt = sizeof(small_modelname) - 1;
	for( i=0; i<cnt; i++ )	small_modelname[i] = tolower(modelname[i]);
	small_modelname[i] = '\0';
